...
Router.route('/organization/:id/invite', {
    ...
    onRun: function(){
        var o = _.findWhere(AccountsTemplates._fields, {_id: "email"});
        if (o.visible && o.visible.indexOf("inviteNew") === -1) {
          o.visible.push("inviteNew")
          AccountsTemplates.texts.button.inviteNew = "Send invitation";
          AccountsTemplates.texts.title.inviteNew = "Send invitation";
          console.log("Patched", _.findWhere(AccountsTemplates._fields, {_id: "email"}))
        }
        this.next();
    }
})
